...
A decision log of the spam-operator → `NorskHelsenett/spam` rollout, written
...
- [`architecture.md`](architecture.md) — why the operator looks the way it does
...
## What's already landed in the operator
...
- Clusters appear and disappear automatically — the operator ships inside an
...
- We cannot ship a long-lived secret inside the operator's container image
...
│                                                                   │
│   umbrella Helm chart installs spam-operator                      │
│   │                                                               │
│   ServiceAccount "spam-operator"                                  │
│   │ projected token volume with audience: spam.nhn.no             │
│   │ (kubelet rotates every hour, no human touches it)             │
│   ▼                                                               │
│   operator POST → https://spam.nhn.no/api/ingest/operator/records │
...
└───────────────────────────────────────────────────────────────────┘
                                │
                                ▼
                 operator picks up token on next restart,
                 prefers it over the SA JWT going forward
```

Post-approval, the per-cluster token is what the operator sends. It's
...
| T3 | Compromised umbrella Git repo plants an operator with malicious code | Legit cluster, malicious payload | Out of scope — supply-chain problem upstream of us |
...
5. **Mismatch sanity checks.** Auto-reject if `sub` is not
   `system:serviceaccount:spam-operator:spam-operator` (i.e., not our
   operator's SA), or `aud` is not the expected `spam.nhn.no`. A malicious
   actor deploying their own operator with their own SA gets auto-rejected
   at the claims level.
...
- `POST /api/ingest/operator/records` — ndjson body, bearer-token auth,
  rate-limited.
- `POST /api/ingest/operator/heartbeat` — keeps `clusters.last_seen_at`
  fresh.
...
- Operator-restart reconcile: on ingest `reconcile_begin` marker, mark
...
1. **Operator transport**: swap stdout for HTTP POST ndjson. Config:
   `SPAM_URL` env, `/var/run/secrets/tokens/spam` token path. Token
   precedence: per-cluster token file > projected SA JWT.
2. **Operator heartbeat**: goroutine, 60s tick, `{cluster, summary counts}`.
3. **Operator NetworkPolicy**: default-deny + egress to spam only + DNS +
   kube-apiserver.
4. **spam schema**: `clusters`, `cluster_tokens`, `pods`, `pod_containers`,
   `cluster_ingresses`. Extend `image_digests`.
5. **spam ingest**: `/api/ingest/operator/records` + heartbeat + pending
...
11. **spam reconcile marker on operator restart** (belt for suspenders).
...
- Cosign image signature verification on the operator side.
...
- **Per-cluster token delivery.** Admin approves → spam generates token →
  ...
  exposes a one-shot retrieval URL that the operator can hit while still
...
- **Heartbeat summary content.** How much does the heartbeat carry? At
  ...
  cheap consistency check (detect drift between operator and spam), but
...
- **Rate limits.** What's the budget for pending clusters? Proposal: 30
  ...
  in spam config.
- **Reconcile marker wire format.** Operator sends
...
- **Terminal pod emits.** Current design keeps `pod_phase` visible on every
  ...
  operator flag `OMIT_TERMINAL_PHASES=Succeeded,Failed`. YAGNI until
  proven.
- **Image signature verification.** Cosign-verify the operator image at
...
- Operator is done and running. Stream is clean: ADD / UPDATE / DELETE,
